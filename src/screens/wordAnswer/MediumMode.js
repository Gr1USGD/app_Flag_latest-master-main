...
const MediumMode = ({ navigation, route }) => {
    ...
    const updateScoreBoard = async () => {
        ...
        if (correctCount >= user.picGuess.medium) {
            await updateScore(currentUser.uid,
                {
                    ...
                    picGuess: {
                        ...user.picGuess,
                        medium: correctCount,
                        total: user.picGuess.total += (correctCount * 2),
                        correctAnswer: user.picGuess.correctAnswer += correctCount,
                        incorrectAnswer: user.picGuess.incorrectAnswer += incorrectCount,
                    }
                })
    ...
    useEffect(() => {
        if (showScore) {
            ...
            navigation.navigate('Result', { score: correctCount * 2, mode: 'MediumMode', playMode: true });
        }
    }, [showScore]);
        ...
        if (selectedAnswer === null || selectedAnswer !== item) {
            ...
            if (answer === item) {
                ...
                if (!selecWrongAnswers.includes(item)) {
                    ...
                    if (heartCount === 1) {
                        setShowScore(true);
                        navigation.navigate('Result', { mode: 'MediumMode', score: correctCount * 2, playMode: true });
                    }
                }
            }
        }
