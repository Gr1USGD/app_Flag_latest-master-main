    ...
    const PlayMode = ({ navigation, route }) => {
        ...
        return (
            <View style={style.body}>
                ...
                <View style={style.mode}>
                    <TouchableOpacity style={style.easybtn} onPress={() => navigation.navigate('EasyMode', { quizId: allQuizzes.find(quiz => quiz.mode === 'easy')?.id, currentUser: currentUser })}>
                        <Text style={[style.text, { position: 'absolute', marginLeft: '15%' }]}>DỄ</Text>
                        <Image style={style.easy} source={require('../../../assets/easyicon.png')} />
                    </TouchableOpacity>

                    <TouchableOpacity style={style.mediumbtn} onPress={() => navigation.navigate('MediumMode', { quizId: allQuizzes.find(quiz => quiz.mode === 'medium')?.id, currentUser: currentUser })}>
                    ...
                    <TouchableOpacity style={style.hardbtn} onPress={() => {
                        if (refreshing) {
                            navigation.navigate('HardMode', { quizId: allQuizzes.find(quiz => quiz.mode === 'hard')?.id, currentUser: currentUser });
                        }
                    }}>
