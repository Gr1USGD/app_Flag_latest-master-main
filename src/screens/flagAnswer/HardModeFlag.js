...
import { updateAnswer, getQuestionsByQuizId_2, updateScore, getUserById } from '../../utils/Database';
import LottieView from "lottie-react-native";
LogBox.ignoreAllLogs();
const HardModeFlag = ({ navigation, route }) => {
    ...
    const updateScoreBoard = async () => {
        ...
        if (correctCount >= user.textGuess.hard) {
            await updateScore(currentUser.uid,
                {
                    ...
                    textGuess: {
                        ...user.textGuess,
                        hard: correctCount,
                        total: user.textGuess.total += (correctCount * 3),
                        correctAnswer: user.textGuess.correctAnswer += correctCount,
                        incorrectAnswer: user.textGuess.incorrectAnswer += incorrectCount,
                    }
                })
    ...
    useEffect(() => {
        if (showScore) {
            ...
            navigation.navigate('Result', { score: correctCount * 3, mode: 'HardModeFlag', playMode: false });
        }
    }, [showScore]);
        ...
        if (selectedAnswer === null || selectedAnswer !== item) {
            ...
            if (answer === item) {
                ...
                if (!selecWrongAnswers.includes(item)) {
                    ...
                    if (heartCount === 1) {
                        setShowScore(true);
                        navigation.navigate('Result', { mode: 'HardModeFlag', score: correctCount * 3, playMode: false });
                    }
                }
            }
        }
    ...
    const preLoadImages = async (imageUrls) => {
        try {
            if (imageUrls && Array.isArray(imageUrls)) {
                // Tải trước từng ảnh trong danh sách
